      ...
      <div className="w-full max-w-5xl bg-white rounded-3xl shadow-2xl flex flex-col md:flex-row overflow-hidden min-h-[500px]">
        ...
        <div className="hidden md:flex w-5/12 bg-gradient-to-br from-indigo-700 to-indigo-500 relative p-12 text-white">
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-8 z-10">
            ...
            <p className="text-indigo-100 font-medium mb-10 text-lg">Your Gateway to Internships & Skill Development</p>
            <h3 className="text-xl font-bold text-white mb-1">Welcome Back!</h3>
            <p className="text-indigo-200 text-base font-light max-w-xs">Continue your career journey where you left off.</p>
          </div>
          ...
          <div className="absolute bottom-4 left-0 right-0 text-center text-xs text-indigo-300">© 2025 Prashikshan Inc.</div>
        </div>

        {/* RIGHT SIDE - FORM */}
        <div className="w-full md:w-7/12 p-8 md:p-10 lg:p-12 bg-white flex justify-center items-center">
          <div className="w-full max-w-sm">
            <h2 className="text-3xl font-bold text-slate-900 mb-2 text-center">Student Login</h2>
            <p className="text-slate-500 mb-8 text-center">Sign in to access your dashboard.</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  disabled={loading}
                  className="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 block p-3 transition-all disabled:opacity-50"
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    disabled={loading}
                    className="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 block p-3 pr-16 transition-all disabled:opacity-50"
              ...
              <div className="flex items-center gap-3 pt-1">
                <button
                  ...
                  className={`flex items-center justify-center w-6 h-6 rounded border-2 transition-all flex-shrink-0 mt-0.5 ${
                    rememberMe ? 'bg-indigo-600 border-indigo-600' : 'border-slate-300 hover:border-indigo-500'
                  }`}
                >
                  {rememberMe && (
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-indigo-600 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-indigo-500/30 transition-all active:scale-[0.98] hover:bg-indigo-700 mt-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <div className="mt-6 text-center pt-2">
                <Link to="/studentsignup" className="text-sm text-slate-500">
                  ...
                  <span className="text-indigo-600 font-bold hover:underline ml-1">Sign up instead</span>
                </Link>
              </div>
            </form>
          </div>
